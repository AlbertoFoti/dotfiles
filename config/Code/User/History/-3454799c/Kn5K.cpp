...
void GE_Guidance_class::buildMissionTrajectory() {
	...
	while(stop == false) {
		...
		double planetRadius = 3400000; 
		double deg2km = (2*PIG*planetRadius/1000)/360;
		double err_max = 10;

		double error_AT = (LS.lon - PDP.lon)*deg2km;
		double error_CT = (LS.lat - PDP.lat)*deg2km;

		printf("Error Along Track (AT) [km] : %lf\n", error_AT);
		printf("Error Cross Track (CT) [km] : %lf\n", error_CT);

		double error_norm = sqrt(pow(error_AT, 2) + pow(error_CT, 2));
		printf("Distance from LS [km] : %lf\n", error_norm);
