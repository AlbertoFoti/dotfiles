...
MatrixT::MatrixT(std::vector<std::array<double, 6>> data) {
...
MatrixT MatrixT::transpose(MatrixT m) {
    ...
    int cols = m.data.at(0).size();

    MatrixT transposed;
    transposed.data.resize(cols, std::vector<double>(rows));

    for (int i = 0; i < rows; ++i) {
      for (int j = 0; j < cols; ++j) {
        transposed.data.at(j).at(i) = m.data.at(i).at(j);
      }
    }
...
void MatrixT::fillRow(size_t ix_row, std::vector<double> row) {
    this->data.at(ix_row).clear();
    this->data.at(ix_row) = row;
}
...
void MatrixT::insertRow(std::vector<double> row, size_t ix_row) {
    if(data.size() <= ix_row) {
        this->data.push_back(row);
    } else {
        for(unsigned int j=0; j<row.size(); j++) {
            this->data.at(ix_row).at(j) = row.at(j);
        }
    }
}

void MatrixT::insertCol(std::vector<double> col, size_t ix_col) {
    // fill rows if non-existent
    for(unsigned int i=data.size(); i<col.size(); i++) {
        std::vector<double> row {};
        data.push_back(row);
    }

    for(unsigned int i=0; i<col.size(); i++) {
        // fill column up to ix_col if non-existent
        for(unsigned int j=data.at(i).size(); j<ix_col; j++) {
            data.at(i).push_back(0.00);
        }

        // Insert element
        data.at(i).push_back(col.at(i));
    }
}
...
MatrixT MatrixT::matrixSub(MatrixT m1, MatrixT m2) {
    MatrixT res;
    res.data = m1.data;
...
std::vector<double> MatrixT::getRow(uint32_t row) {
    return this->data.at(row);
}
...
std::vector<double> MatrixT::getLastRow() {
