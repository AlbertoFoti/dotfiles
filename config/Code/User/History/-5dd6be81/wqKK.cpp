...
void GEPathPlanning::buildMissionTrajectory() {
    ...
    while (stop == false) {
        ...
        // Saving results
        std::vector<double> r_profile;
        std::vector<double> lat_profile;
        std::vector<double> lon_profile;
        std::vector<double> vel_profile;
        std::vector<double> head_profile;
        std::vector<double> fpa_profile;
        for(unsigned int i=0; i<nominal_traj.xCoM.size().at(0); i++) {
            r_profile.push_back(nominal_traj.xCoM.at(i).at(0));
            lon_profile.push_back(nominal_traj.xCoM.at(i).at(1));
            lat_profile.push_back(nominal_traj.xCoM.at(i).at(2));
            vel_profile.push_back(nominal_traj.xCoM.at(i).at(3));
            head_profile.push_back(nominal_traj.xCoM.at(i).at(4));
            fpa_profile.push_back(nominal_traj.xCoM.at(i).at(5));
        }
        
        writeTo("Data/r_nominal_profile.txt", r_profile);
        writeTo("Data/lat_nominal_profile.txt", lat_profile);
        writeTo("Data/lon_nominal_profile.txt", lon_profile);
        writeTo("Data/vel_nominal_profile.txt", vel_profile);
        writeTo("Data/head_nominal_profile.txt", head_profile);
        writeTo("Data/fpa_nominal_profile.txt", fpa_profile);
...
BRMsData GEPathPlanning::buildNominalMissionTrajectory() {
    ...
    ms->NominalMissionNSF.TNom = ms->TimeStepsNSF.Tgui;
