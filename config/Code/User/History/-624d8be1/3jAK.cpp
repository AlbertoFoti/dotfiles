...
void GEPathPlanning::buildGuidanceTrajectory() {
    ...
    if(ms->CoMControlParsNSF.GuiFlag == false) {
        ...
        ms->gui_profile_out.muSim = nominal_traj.mu;
        // muNomNBR
    ...
    // write to file
    writeGuidanceData("Data/GEpathplannerLong.DHS", ms->gui_profile_out);
...
bool GEPathPlanning::writeGuidanceData(const std::string& filename, GuidanceProfileOutStr gui_profile) {
    ...
    auto fp1 = fopen(filename.c_str(),"w");
    ...
    for(unsigned int i=0; i<ms->gui_profile_out.xCoMRs.size(); i++) {
        for(unsigned int j=0; j<5; j++) {
            fprintf(fp1,"%.6g ", ms->gui_profile_out.xCoMRs.at(j));
        }
        fprintf(fp1,"%.6g\n", ms->gui_profile_out.xCoMRs.at(5));
    }
    fprintf(fp1,"%.6g %.6g %.6g %.6g %.6g %.6g\n", ms->gui_profile_out.xCoMRs);
