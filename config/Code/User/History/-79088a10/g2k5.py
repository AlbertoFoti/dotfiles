...
ax = fig.add_subplot(gs[0:4, 0:2], projection='3d')
ax.view_init(elev=35, azim=-120)  # Adjust these for desired view
ax.set_box_aspect([1, 1, 1])

# Set starting point and direction vector
x0, y0, z0 = 0, 0, 0  # starting point
u, v, w = 2, 3, 1  # direction vector
...
roll = np.genfromtxt("mu_profile.txt", delimiter="\n")
pitch = np.zeros(len(roll))
yaw   = np.zeros(len(roll))

# 2D subplot for real-time data
ax2 = fig.add_subplot(gs[0:2, 2:4])
ax2.set_xlim([0, len(roll)])
ax2.set_ylim([min(roll), max(roll)])
x_data, y_data = [], []
line2, = ax2.plot([], [], 'r-')

ax3 = fig.add_subplot(gs[2:4, 2], projection='3d')
ax3.view_init(elev=15, azim=-60)
ax3.grid(True)

ax4 = fig.add_subplot(gs[2:4, 3])
ax4.set_xlim([0, len(roll)])
...
while t < 290:
    ...
    dm.updateAttitude(roll[t], pitch[t], yaw[t])
    ...
    ax3.quiver(0, -14, 0, 5, 0, 0, color='green')    
    ax3.quiver(0, -14, 0, 0, 5, 0, color='green')    
    ax3.quiver(0, -14, 0, 0, 0, 5, color='green')    

    # Plot the velocity vector
    ax3.quiver(x0, y0, z0, u, v, w, color='red')
    ...
    x_data.append(t)
    y_data.append(roll[t])  # Random data for demonstration
    ...
    ax2.set_ylim([min(roll)-1, max(roll)+1])  # Keep y-axis fixed between 0 and 100
    ax2.grid(True)
