...
void GEPathPlanning::simCoM3dof() {
    ...
    while(t_count < Tsim) {
        // Grav(R) -> gNED
        std::array<double, 3> gNED = {0, 0, 3.0}; // take this from Grav block output instead
        double Vw = xCoM.at(3);
        double R  = xCoM.at(0);
    ...
    std::cout << "  Error along track (AT) [km] = 0.00"; //<< err_AT;
    std::cout << "  Error cross track (CT) [km] = 0.00"; //<< err_CT;
    std::cout << "  Distance from LS [km]       = 0.00"; //<< norm(err_AT, err_CT);
    std::cout << "-------------------------------------\n";
    return;
}
